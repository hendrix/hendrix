...
class _ThroughToYou(object):
    ...
    def __call__(self, crosstown_task=None):
        ...
        try:
            ...
            self.log.info(
                "Adding '%s' to crosstown_traffic for %s"
                % (str(crosstown_task), self.response)
            )
            if not self.no_go:
    ...
    def responseless_fallback(self, crosstown_task=None):
        ...
        if self.fail_without_response:
            ...
            self.log.info(
                "thread %s has no response; running crosstown task now.  To supress this behavior, set fail_without_response == True."
    ...
    def check_status_code_against_no_go_list(self):
        if self.no_go_status_codes:
            ...
            if self.status_code in self.no_go_status_code_list:
                self.log.info(
                    "Flagging no-go becasue status code is %s" % self.status_code
                )
                self.no_go = True
