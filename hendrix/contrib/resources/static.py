...
def generate_resources_for_location(disk_root, url):
    for root, dirs, files in os.walk(disk_root):
        yield DjangoStaticResource(
            root, url.strip("/") + "%s" % root.split(disk_root)[-1]
        )
...
class DjangoStaticsFinder(object):
    ...
    def get_resources():
        ...
        if not settings.DEBUG or has_pipeline_finder:
            yield DjangoStaticResource(settings.STATIC_ROOT, settings.STATIC_URL)
        else:
            existing = []
...
try:
    ...
    raise AttributeError(
        "Please make sure you have assigned your STATIC_ROOT and STATIC_URL" " settings"
    )
